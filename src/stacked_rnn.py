...
class StackedRNNMusic:
    ...
    def forward_prop(self):
        ...
        for i in range(len(self.num_units)):
            cell_dict['lstm_cell_'+str(i+1)] = tf.contrib.rnn.LSTMCell(num_units=self.num_units[i], initializer=tf.contrib.layers.xavier_initializer(), activation=tf.nn.relu)
        stacked_cell = tf.contrib.rnn.MultiRNNCell([cell_dict['lstm_cell_'+str(i+1)] for i in range(len(self.num_units))])
        output, state = tf.nn.dynamic_rnn(stacked_cell, self.inputs, dtype=tf.float32)
